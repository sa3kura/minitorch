...
void launch_attn_softmax_bw(float *out_grad,
  ...
  cudaMalloc((void **)&d_out_grad, inp_size);
  cudaMalloc((void **)&d_soft_outp, inp_size);

  cudaMemcpy(d_out_grad, out_grad, inp_size, cudaMemcpyHostToDevice);
  ...
  if (softmax_len <= 32) {
    ...
    throw std::runtime_error("softmax length greater than 1024 is not supported");
  }

  cudaMemcpy(out_grad, d_out_grad, inp_size, cudaMemcpyDeviceToHost);
  cudaDeviceSynchronize();
